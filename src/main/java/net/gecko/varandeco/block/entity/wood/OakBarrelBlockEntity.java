...
public class OakBarrelBlockEntity extends LootableContainerBlockEntity {
	private DefaultedList<ItemStack> inventory = DefaultedList.ofSize(27, ItemStack.EMPTY);
	private final ViewerCountManager stateManager = new ViewerCountManager() {
		@Override
		protected void onContainerOpen(World world, BlockPos pos, BlockState state) {
			OakBarrelBlockEntity.this.playSound(state, SoundEvents.BLOCK_BARREL_OPEN);
			OakBarrelBlockEntity.this.setOpen(state, true);
		}

		@Override
		protected void onContainerClose(World world, BlockPos pos, BlockState state) {
			OakBarrelBlockEntity.this.playSound(state, SoundEvents.BLOCK_BARREL_CLOSE);
			OakBarrelBlockEntity.this.setOpen(state, false);
		}
		...
		protected boolean isPlayerViewing(PlayerEntity player) {
			if (player.currentScreenHandler instanceof GenericContainerScreenHandler) {
				...
				return inventory == OakBarrelBlockEntity.this;
			} else {
				return false;
			}
		}
	...
	public OakBarrelBlockEntity(BlockPos pos, BlockState state) {
		super(DecoBlockEntities.OAK_BARREL_BE, pos, state);
	}
