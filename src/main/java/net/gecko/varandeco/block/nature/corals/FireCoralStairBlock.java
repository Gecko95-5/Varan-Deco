...
public class FireCoralStairBlock extends StairsBlock {
    public FireCoralStairBlock(BlockState baseBlockState, Settings settings) {
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!isInWater(state, world, pos)) {
            world.setBlockState(pos, DecoBlocks.DEAD_FIRE_CORAL_STAIRS.getDefaultState().with(WATERLOGGED, false).with(FACING, state.get(FACING)), Block.NOTIFY_LISTENERS);
        }
    }
