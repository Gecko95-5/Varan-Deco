...
public class BubbleCoralStairBlock extends StairsBlock {
    public BubbleCoralStairBlock(BlockState baseBlockState, Settings settings) {
    ...
    protected void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!isInWater(state, world, pos)) {
            world.setBlockState(pos, DecoBlocks.DEAD_BUBBLE_CORAL_STAIRS.getDefaultState().with(WATERLOGGED, false).with(FACING, state.get(FACING)), Block.NOTIFY_LISTENERS);
        }
    }
