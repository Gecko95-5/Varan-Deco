...
public class EndstoneStonecutterScreenHandler extends ScreenHandler {
    ...
    public final Inventory input = new SimpleInventory(1) {
        ...
        public void markDirty() {
            super.markDirty();
            EndstoneStonecutterScreenHandler.this.onContentChanged(this);
            EndstoneStonecutterScreenHandler.this.contentsChangedListener.run();
        }
    };
    final CraftingResultInventory output = new CraftingResultInventory();

    public EndstoneStonecutterScreenHandler(int syncId, PlayerInventory playerInventory) {
        this(syncId, playerInventory, ScreenHandlerContext.EMPTY);
    }

    public EndstoneStonecutterScreenHandler(int syncId, PlayerInventory playerInventory, ScreenHandlerContext context) {
        ...
        this.outputSlot = this.addSlot(new Slot(this.output, 1, 143, 33) {
            ...
            public void onTakeItem(PlayerEntity player, ItemStack stack) {
                ...
                EndstoneStonecutterScreenHandler.this.output.unlockLastRecipe(player, this.getInputStacks());
                ItemStack itemStack = EndstoneStonecutterScreenHandler.this.inputSlot.takeStack(1);
                if (!itemStack.isEmpty()) {
                    EndstoneStonecutterScreenHandler.this.populateResult(EndstoneStonecutterScreenHandler.this.selectedRecipe.get());
                }

                context.run((world, pos) -> {
                    long l = world.getTime();
                    if (EndstoneStonecutterScreenHandler.this.lastTakeTime != l) {
                        world.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
                        EndstoneStonecutterScreenHandler.this.lastTakeTime = l;
                    }
                });
            ...
            private List<ItemStack> getInputStacks() {
                return List.of(EndstoneStonecutterScreenHandler.this.inputSlot.getStack());
            }
        });
    ...
    public boolean canUse(PlayerEntity player) {
        return canUse(this.context, player, DecoBlocks.END_STONE_STONECUTTER);
    }
