...
public class DeepslateStonecutterScreenHandler extends ScreenHandler {
	...
	public final Inventory input = new SimpleInventory(1) {
		...
		public void markDirty() {
			super.markDirty();
			DeepslateStonecutterScreenHandler.this.onContentChanged(this);
			DeepslateStonecutterScreenHandler.this.contentsChangedListener.run();
		}
	};
	final CraftingResultInventory output = new CraftingResultInventory();

	public DeepslateStonecutterScreenHandler(int syncId, PlayerInventory playerInventory) {
		...
		this.outputSlot = this.addSlot(new Slot(this.output, 1, 143, 33) {
			...
			public void onTakeItem(PlayerEntity player, ItemStack stack) {
				...
				context.run((world, pos) -> {
					...
					if (DeepslateStonecutterScreenHandler.this.lastTakeTime != l) {
						world.playSound(null, pos, SoundEvents.UI_STONECUTTER_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
						DeepslateStonecutterScreenHandler.this.lastTakeTime = l;
					}
				});
