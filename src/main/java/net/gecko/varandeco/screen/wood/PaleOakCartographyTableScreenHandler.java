...
public class PaleOakCartographyTableScreenHandler extends ScreenHandler {
	...
	public final Inventory inventory = new SimpleInventory(2) {
		...
		public void markDirty() {
			PaleOakCartographyTableScreenHandler.this.onContentChanged(this);
			super.markDirty();
		}
	};
	...
	public PaleOakCartographyTableScreenHandler(int syncId, PlayerInventory inventory) {
		this(syncId, inventory, ScreenHandlerContext.EMPTY);
	}

	public PaleOakCartographyTableScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context) {
		...
		this.addSlot(new Slot(this.resultInventory, 2, 145, 39) {
			...
			public void onTakeItem(PlayerEntity player, ItemStack stack) {
				PaleOakCartographyTableScreenHandler.this.slots.get(0).takeStack(1);
				PaleOakCartographyTableScreenHandler.this.slots.get(1).takeStack(1);
				stack.getItem().onCraft(stack, player.getWorld());
				context.run((world, pos) -> {
					long l = world.getTime();
					if (PaleOakCartographyTableScreenHandler.this.lastTakeResultTime != l) {
						world.playSound(null, pos, SoundEvents.UI_CARTOGRAPHY_TABLE_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
						PaleOakCartographyTableScreenHandler.this.lastTakeResultTime = l;
					}
				});
	...
	public boolean canUse(PlayerEntity player) {
		return canUse(this.context, player, DecoBlocks.PALE_OAK_CRAFTING_TABLE);
	}
