...
public class DriftwoodCartographyTableScreenHandler extends ScreenHandler {
	...
	public final Inventory inventory = new SimpleInventory(2) {
		...
		public void markDirty() {
			DriftwoodCartographyTableScreenHandler.this.onContentChanged(this);
			super.markDirty();
		}
	};
	...
	public DriftwoodCartographyTableScreenHandler(int syncId, PlayerInventory inventory) {
		this(syncId, inventory, ScreenHandlerContext.EMPTY);
	}

	public DriftwoodCartographyTableScreenHandler(int syncId, PlayerInventory inventory, ScreenHandlerContext context) {
		...
		this.addSlot(new Slot(this.resultInventory, 2, 145, 39) {
			...
			public void onTakeItem(PlayerEntity player, ItemStack stack) {
				DriftwoodCartographyTableScreenHandler.this.slots.get(0).takeStack(1);
				DriftwoodCartographyTableScreenHandler.this.slots.get(1).takeStack(1);
				stack.getItem().onCraft(stack, player.getWorld());
				context.run((world, pos) -> {
					long l = world.getTime();
					if (DriftwoodCartographyTableScreenHandler.this.lastTakeResultTime != l) {
						world.playSound(null, pos, SoundEvents.UI_CARTOGRAPHY_TABLE_TAKE_RESULT, SoundCategory.BLOCKS, 1.0F, 1.0F);
						DriftwoodCartographyTableScreenHandler.this.lastTakeResultTime = l;
					}
				});
	...
	public boolean canUse(PlayerEntity player) {
		return canUse(this.context, player, DecoBlocks.DRIFTWOOD_CARTOGRAPHY_TABLE);
	}
